<script lang="ts">
  ...
  let safeLives = $derived(Math.max(0, Math.floor($playerLives || 0)))
  let safeHealth = $derived(Math.max(0, Math.min(maxHealth, $playerHealth || 0)))
