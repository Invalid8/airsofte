<script lang="ts">
  ...
  import type { BonusObjective } from '$lib/game/objectiveTracker'
  import type { StoryMission } from '$lib/types/gameTypes'

  type MissionObjective = StoryMission['objectives'][number]
  ...
  let objectives = $state<MissionObjective[]>([])
  let bonusObjectives = $state<BonusObjective[]>([])
  ...
  function getObjectiveProgress(objective: MissionObjective): number {
    return Math.min((objective.current / objective.target) * 100, 100)
  }

  function getObjectiveColor(objective: MissionObjective): string {
