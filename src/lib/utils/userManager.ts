...
import type { UserProfile, UserSession } from '$lib/types/userTypes'
...
class UserManager {
  ...
  async createUser(
    ...
    if (isGuest) {
      this.sessionStore.set({ currentUser: newUser, isLocked: false })
      this.saveSession({ currentUser: newUser, isLocked: false })
    } else {
      users.push(newUser)
  ...
  async switchUser(userId: string, password?: string): Promise<boolean> {
    const users = this.getUsersFromStorage()
    const guestUser = this.getSession().currentUser
    const user =
      users.find((u) => u.id === userId) ||
      (guestUser?.isGuest && guestUser.id === userId ? guestUser : undefined)
