...
<div class="hud fixed top-0 left-0 right-0 p-6 pointer-events-none z-50">
  <div class="flex justify-between items-start gap-4">
    ...
    <div class="right-panel space-y-3 text-right">
      <div class="lives-display flex items-center justify-end gap-2">
        <div class="label text-sm opacity-70">LIVES</div>
      ...
      <div class="health-display">
        <div class="label text-sm opacity-70 mb-1">HEALTH</div>
        <div
        ...
        <div class="health-value text-sm mt-1 hud">{$playerHealth} / {maxHealth}</div>
      </div>
      <div class="h-3"></div>

      <div class="weapon-display">
        <div class="label text-sm opacity-70">WEAPON</div>
        <div class="value text-xl font-bold hud">{weaponType}</div>
      </div>

      {#if invincible}
        <div class="invincible-indicator text-sm text-cyan-400 animate-pulse hud">INVINCIBLE</div>
      {/if}
    </div>
  </div>
</div>
